...
namespace mcmtestOpenTK.ServerSystem.CommandHandlers
{
    public abstract class AbstractCommand
    {
        ...
        {
            // TODO: Format for server-stuff!
            SysConsole.Output(OutputType.SERVERINFO, TextStyle.Color_Separate + info.Command.Name + TextStyle.Color_Outgood + ": " + info.Command.Description);
            SysConsole.Output(OutputType.SERVERINFO, TextStyle.Color_Commandhelp + "Usage: /" + info.Name + " " + info.Command.Arguments);
            if (info.Command.IsDebug)
            {
                SysConsole.Output(OutputType.SERVERINFO, "Note: This command is intended for debugging purposes.");
            }
        }
    }
}
