        ...
        public override void TickMe()
        {
            lock (Locker)
            {
                if (Error != null)
                {
                    ...
                    KillQuietly = true;
                    SysConsole.Output(OutputType.ERROR, "Global Time Request failed with message: " + TextStyle.Color_Separate + Error);
                    return;
                }
            }
            if (socket.Connected)
            {
                ...
                if (avail > 0)
                {
                    ...
                    if (ShouldAnnounce)
                    {
                        SysConsole.Output(OutputType.INFO, "Response from Global Server: " + TextStyle.Color_Separate + Time);
                    }
                    ready = true;
                }
            }
