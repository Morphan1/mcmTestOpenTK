...
namespace mcmtestOpenTK.Client.CommandHandlers.QueueCmds
{
    class ElseCommand: AbstractCommand
    {
        ...
        public override void Execute(CommandInfo info)
        {
            ...
            if (IfEntry == null)
            {
                UIConsole.WriteLine(TextStyle.Color_Outbad + "ELSE invalid: IF command did not preceed!");
                return;
            }
            if (IfEntry.Info.Result == 1)
            {
                UIConsole.WriteLine(TextStyle.Color_Outbad + "ELSE continuing, IF passed.");
                return;
            }
            if (info.Arguments.Count >= 1)
            {
                ...
                else
                {
                    ...
                    if (info.Entry.Block != null)
                    {
                        if (success)
                        {
                            UIConsole.WriteLine(TextStyle.Color_Outgood + "Else if is true, executing...");
                            info.Result = 1;
                            info.Queue.AddCommandsNow(info.Entry.Block);
                        }
                        else
                        {
                            UIConsole.WriteLine(TextStyle.Color_Outgood + "Else If is false, doing nothing!");
                        }
                    }
                }
            }
            else
            {
                if (info.Entry.Block != null)
                {
                        UIConsole.WriteLine(TextStyle.Color_Outgood + "Else is valid, executing...");
                        info.Result = 1;
                        info.Queue.AddCommandsNow(info.Entry.Block);
                }
                else
                {
                    UIConsole.WriteLine(TextStyle.Color_Outbad + "ELSE invalid: No block follows!");
                }
            }
        }
    }
