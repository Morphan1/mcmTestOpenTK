...
namespace mcmtestOpenTK.Client.CommandHandlers
{
    public abstract class AbstractCommand
    {
        ...
        {
            UIConsole.WriteLine(TextStyle.Color_Separate + info.Command.Name + TextStyle.Color_Outgood + ": " + info.Command.Description);
            UIConsole.WriteLine(TextStyle.Color_Commandhelp + "Usage: /" + info.Name + " " + info.Command.Arguments);
            if (info.Command.IsDebug)
            {
                UIConsole.WriteLine("Note: This command is intended for debugging purposes.");
            }
        }
    }
}
